...
int main(int argc, char **argv)
{
	...
	float first_time=(float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000);
	fprintf(stderr, "Execution reference time = %fs\n",first_time );
    ...
    gettimeofday(&stop, NULL);
    float final_timer= (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000);
	fprintf(stderr, "Execution using omp time = %fs\n", final_timer);
    exit(EXIT_SUCCESS);
}
