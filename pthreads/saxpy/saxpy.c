 ...
 *
 * Student names: Charles Tran - 14377220
 * Date: 4/15/2023
 *
 * */
...
void *saxpy_v1(void *arg);
void *saxpy_v2(void *arg);
...
void compute_using_pthreads_v1(float *x, float *y, float a, int num_elements, int num_threads)
{
    ...
    // chunking method
    // chunk_size = n (num_elements) / k (num_threads)
    // offset within A and B = tid * chunk_size 
    // Threads 0 to k-2: offset to offset + chunk size
    // last thread (tid = k-1) -> offset to n-1
...
void compute_using_pthreads_v2(float *x, float *y, float a, int num_elements, int num_threads)
{
    /* FIXME: Complete this function */
    // striding method
    // each thread strides over elements of A and B
    // tid = thread ID, k = stride length, k = # of threads
    pthread_t *thread_id = (pthread_t *)malloc(num_threads *sizeof(pthread_t));     // Data structure to store the thread IDs
    pthread_attr_t attributes;                                                      // Thread Attributes
    pthread_attr_init(&attributes);                                                 // Initialize thread attributes to default values
