...
{
    compute_naive(A, gpu_naive_sol_x, B);
    compute_optimize(A, gpu_opt_sol_x, B);
    return;
}

void compute_optimize(const matrix_t A, matrix_t gpu_opt_solution_x, const matrix_t B) {
    struct timeval start, stop;	
    int num_threads;
    double mse, ssd, *d_ssd;
    unsigned int done = 0;
    int num_iter = 0;
    int i, j;
    float *temp;
    matrix_t A_transpose = allocate_matrix_on_host(A.num_rows, A.num_columns, 0);
    for (i = 0; i < A_transpose.num_rows; i++) {
        for (j = 0; j < A_transpose.num_columns; j++) {
            A_transpose.elements[i * A_transpose.num_columns + j] = A.elements[j * A_transpose.num_columns + i];
        }
    }

    /* Load matrices A and X on to device */
    matrix_t Ad = allocate_matrix_on_device(A_transpose);
    copy_matrix_to_device(Ad, A_transpose);

    matrix_t Xd = allocate_matrix_on_device(gpu_opt_solution_x);
	copy_matrix_to_device(Xd, gpu_opt_solution_x);

    matrix_t Xd_new = allocate_matrix_on_device(gpu_opt_solution_x);
    copy_matrix_to_device(Xd_new, gpu_opt_solution_x);
    ...
    dim3 threads(1, num_threads, 1);
    dim3 grid(1, (Bd.num_rows + num_threads - 1)/num_threads);
    
    gettimeofday(&start, NULL);
    while (!done) {
        cudaMemset(d_ssd, 0, sizeof(double));
        jacobi_iteration_kernel_optimized<<<grid, threads>>>(Ad.elements, Bd.elements, Xd.elements,Xd_new.elements, Ad.num_rows, Ad.num_columns, d_ssd);
        ...
        temp = Xd.elements;
        Xd.elements = Xd_new.elements;
        Xd_new.elements = temp;
    }
    gettimeofday(&stop, NULL);
    printf("\nOptimize convergence achieved after %d iterations \n", num_iter);
    fprintf(stderr, "Execution time = %fs\n\n", (float)(stop.tv_sec - start.tv_sec +\
            (stop.tv_usec - start.tv_usec)/(float)1000000));
    copy_matrix_from_device(gpu_opt_solution_x, Xd_new);
    free(A_transpose.elements);
    cudaFree(Ad.elements);
    cudaFree(Bd.elements);
    cudaFree(Xd.elements);
    cudaFree(Xd_new.elements);
    cudaFree(d_ssd);
}

void compute_naive(const matrix_t A, matrix_t gpu_naive_sol_x, const matrix_t B) {
	...
	copy_matrix_to_device(Xd, gpu_naive_sol_x);

    matrix_t Xd_new = allocate_matrix_on_device(gpu_naive_sol_x);
    copy_matrix_to_device(Xd_new, gpu_naive_sol_x);
    ...
    while (!done) {
        cudaMemset(d_ssd, 0, sizeof(double));
        jacobi_iteration_kernel_naive<<<grid, threads>>>(Ad.elements, Bd.elements, Xd.elements,Xd_new.elements,Ad.num_rows, Ad.num_columns, d_ssd);
        cudaDeviceSynchronize();
        cudaMemcpy(&ssd, d_ssd, sizeof(double), cudaMemcpyDeviceToHost);
        num_iter++;
        mse = sqrt(ssd);
    ...
    fprintf(stderr, "Execution time = %fs\n", (float)(stop.tv_sec - start.tv_sec +\
            (stop.tv_usec - start.tv_usec)/(float)1000000));
    copy_matrix_from_device(gpu_naive_sol_x, Xd_new);
    cudaFree(Ad.elements);
    cudaFree(Bd.elements);
    cudaFree(Xd.elements);
    cudaFree(Xd_new.elements);
